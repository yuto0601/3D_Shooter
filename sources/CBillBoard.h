...
public:
	void SetM_UV(DirectX::XMFLOAT2 uv[]) {
		m_uv[0] = uv[0];
		m_uv[1] = uv[1];
		m_uv[2] = uv[2];
		m_uv[3] = uv[3];
	...
	// デストラクタ
	//関数にアドレスを入れる等の時、コピーコンストラクタが出来てしまい、そこでも同じデストラクタが
	//出来る。二重にデストラクタが出来て、ないオブジェクトをreleaseしてバグを起こす原因を避けるため、
	//virtualにして、このビルボードだけにデストラクタが発生するようにして、コピーされないようにするため
	//の機能。（二重にreleaseしないための防御機能としてvirtualが置いてある。）
	virtual ~CBillBoard(){
		Dispose();
	}
	//頂点座標の設定
